#!/bin/bash
#SBATCH --job-name stai_fixedsplit_mse
#SBATCH -p GPU4v100
#SBATCH -N 1
#SBATCH -t 100-23:0:00
#SBATCH -o job_%j_fixedsplit_mse.out
#SBATCH -e job_%j_fixedsplit_mse.err
#SBATCH --mail-type ALL
#SBATCH --mail-user jia.yao@utsouthwestern.edu

set -euo pipefail

# Trains the MSE-on-log1p baseline on the leakage-safe fixed split.  Same
# architecture and auxiliary losses as the ZINB main model; only the
# reconstruction term changes.  Used for the "is the statistical likelihood
# actually doing work?" comparison in the STAI-X paper.
...
RUN_ROOT="${REPO_ROOT}/runs/fixedsplit_mse"
...
# Stability tweaks vs. the ZINB main recipe (CLAUDE.md §3.3).  Job 10471260
# regressed val from 0.546 (ep7) to 0.751 (ep8) and triggered the
# all-batches-skipped cascade at ep9.  Tighter clip + longer warmup +
# fixed temperature, with the unchanged full-model LRs.
COMMON_ARGS=(
...
run_experiment() {
  ...
  echo "loss_mode=mse  seed=${seed}  w_contrast=${w_contrast}  w_align=${w_align}"
...
print_banner "Fixed-split MSE baseline"
...
# Matched pair against runs/fixedsplit_v3/:
#   full_mse_seed42        <--> full_seed42
#   zinb_contrast_mse_*    <--> zinb_contrast_seed42
# Every knob except loss_mode matches the ZINB counterparts.
run_experiment "Core" "full_mse_seed42" 42 0.1 0.1
run_experiment "Core" "zinb_contrast_mse_seed42" 42 0.1 0.0

if [[ "${RUN_STABILITY}" == "1" ]]; then
  run_experiment "Stability" "full_mse_seed43" 43 0.1 0.1
  run_experiment "Stability" "full_mse_seed44" 44 0.1 0.1
fi

refresh_master_summary

print_banner "Fixed-split MSE sweep finished"
